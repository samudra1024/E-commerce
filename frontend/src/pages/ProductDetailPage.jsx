...
import { ShoppingCart, Heart, Share, Star, ChevronRight, Check, Truck } from 'lucide-react';
      ...
      if (i <= Math.floor(rating)) {
        ...
        stars.push(
          <span key={i} className="relative">
            ...
            <Star className="absolute top-0 left-0 w-5 h-5 overflow-hidden fill-current text-yellow-400" style={{ clipPath: 'inset(0 50% 0 0)' }} />
          </span>
        );
      } else {
        // Empty star
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
          ...
          {product.images && product.images.length > 0 && (
            <div className="grid grid-cols-5 gap-2">
              {product.images.map((img, index) => (
                <div key={index} className="bg-white rounded-lg overflow-hidden border-2 border-gray-200 cursor-pointer hover:border-blue-500 transition-colors">
                  <img 
                    src={img} 
        ...
        <div>
          ...
          <div className="mb-6">
            {product.discount > 0 ? (
              <div className="flex items-center">
                <span className="text-3xl font-bold text-blue-600 mr-3">${discountedPrice}</span>
          ...
          <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3 mb-6">
            ...
            >
              <Heart className={`h-5 w-5 mr-2 ${isWishlist ? 'fill-current' : ''}`} />
